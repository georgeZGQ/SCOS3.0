...
import es.source.code.adapter.OrderedFoodRecycleAdapter;
...
public class OrderedFoodFragment extends Fragment {
    ...
    //自定义recyclerveiw的适配器
    private OrderedFoodRecycleAdapter foodsRecyclerAdapter;
    Button mybutton;     //下单\结账按钮
    TextView tv_price;     //总价
    TextView tv_sum;    //数量
    int mPage;

    public static OrderedFoodFragment newInstance(int page) {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        int sum_number = 0;
        for(FoodsEntity foods: order_item.getOrderedList()){
...
//        }
        ...
        //创建adapter
        foodsRecyclerAdapter = new OrderedFoodRecycleAdapter(getActivity(), foods_List);
